(function () {
  ...
  document.addEventListener('toggle', function (e) {
    ...
    requestAnimationFrame(function () {
      requestAnimationFrame(function () {
        ...
        var pad = parseFloat(cs.scrollPaddingTop);
        if (!pad || isNaN(pad)) {
          var header = document.querySelector('.site-header');
          pad = (header ? header.getBoundingClientRect().height : 0) + 14;
        }
        var rect = el.getBoundingClientRect();
        var targetY = rect.top + window.scrollY - pad;
  ...
  /* UI Design: prototype figure lightbox (keeps teammate ui-design.html behaviour) */
